...
const projects = [
  ...
  },
  {
    ...
    github: null,
    live: 'https://www.linkedin.com/posts/shaly-s-528308255_uipath-rpa-automation-activity-7388492761651580929-5bi-',
    accent: '#818cf8',
    media: shoppingbot,
    mediaType: 'video',
  },
...
function MediaBox({ project }) {
  return (
    <div style={{
      ...
      position: 'relative',
      background: `linear-gradient(135deg, ${project.accent}10, rgba(255,255,255,0.02))`,
      ...
      {project.media ? (
        ...
        <div style={{
          ...
          alignItems: 'center', justifyContent: 'center', gap: 12, position: 'relative', zIndex: 1,
        }}>
          <div style={{ width: 72, height: 72, borderRadius: '50%', border: `1px dashed ${project.accent}40`, display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
            <svg width="28" height="28" viewBox="0 0 24 24" fill="none" stroke={project.accent} strokeWidth="1.3" opacity="0.5">
              <rect x="3" y="3" width="18" height="18" rx="3"/>
              <circle cx="8.5" cy="8.5" r="1.5"/>
              <path d="M21 15l-5-5L5 21"/>
            </svg>
          </div>
          <span style={{ fontSize: 11, color: project.accent, opacity: 0.4, letterSpacing: '1px', textTransform: 'uppercase' }}>Preview</span>
        </div>
      )}

      {/* Corner accents */}
      <div style={{ position: 'absolute', top: 0, left: 0, zIndex: 2, width: 40, height: 40, borderTop: `2px solid ${project.accent}70`, borderLeft: `2px solid ${project.accent}70`, borderTopLeftRadius: 12 }} />
      <div style={{ position: 'absolute', bottom: 0, right: 0, zIndex: 2, width: 40, height: 40, borderBottom: `2px solid ${project.accent}35`, borderRight: `2px solid ${project.accent}35`, borderBottomRightRadius: 12 }} />
    </div>
  );
}
...
function ProjectCard({ project, index }) {
  ...
  const MediaPanel = (
    <div
      ...
      style={{
        width: '48%', minWidth: 260, flexShrink: 0,
        padding: 20,
        borderRight: mediaOnLeft ? `1px solid rgba(255,255,255,0.06)` : 'none',
        borderLeft:  mediaOnLeft ? 'none' : `1px solid rgba(255,255,255,0.06)`,
        display: 'flex', alignItems: 'stretch', minHeight: 300,
      }}
    >
  ...
  const ContentPanel = (
    <div style={{ flex: 1, padding: '28px 30px', display: 'flex', flexDirection: 'column', gap: 0 }}>
      {/* Header */}
      <div style={{ display: 'flex', alignItems: 'flex-start', justifyContent: 'space-between', marginBottom: 12 }}>
        ...
        <span style={{
          padding: '4px 14px', borderRadius: 100, fontSize: 11, letterSpacing: '.5px',
          background: `${project.accent}15`, border: `1px solid ${project.accent}35`, color: project.accent,
          whiteSpace: 'nowrap',
        }}>{project.tag}</span>
      </div>
      ...
      <div style={{ display: 'flex', gap: 10 }}>
        {project.github && (
          <motion.a
            ...
            whileHover={{ borderColor: `${project.accent}70`, color: project.accent, y: -2 }}
            style={{
              flex: project.live ? 1 : 'none', padding: '9px 16px', borderRadius: 8, textAlign: 'center',
        ...
        {(project.live || project.figma) && (
          <motion.a
            href={project.live || project.figma} target="_blank" rel="noreferrer"
            whileHover={{ borderColor: `${project.accent}70`, color: project.accent, y: -2 }}
            style={{
          ...
          >
            ...
            {project.figma && !project.live ? 'View in Figma' : 'Live Demo'}
          </motion.a>
        )}
      </div>
    </div>
  ...
  return (
    <motion.div
      ...
      whileHover={{
        y: -6,
        boxShadow: `0 32px 80px ${project.accent}14, 0 0 0 1px ${project.accent}20`,
      }}
      style={{
    ...
    >
      {/* Top accent bar */}
      <div style={{ height: 2, background: `linear-gradient(90deg, ${project.accent}, ${project.accent}50, transparent)` }} />

      <div className="project-row" style={{ display: 'flex', flexDirection: 'row', minHeight: 300 }}>
        {mediaOnLeft ? <>{MediaPanel}{ContentPanel}</> : <>{ContentPanel}{MediaPanel}</>}
      </div>
    </motion.div>
  );
}
    ...
    <section id="projects" style={{ position: 'relative', zIndex: 1 }}>
      <style>{`
        @media (max-width: 640px) {
          ...
          .project-media-panel { width: 100% !important; min-width: unset !important; border-right: none !important; border-left: none !important; border-bottom: 1px solid rgba(255,255,255,0.06) !important; min-height: 220px !important; }
        }
      `}</style>
      ...
      <div style={{ maxWidth: 1000, margin: '0 auto', padding: '100px 40px' }}>
        ...
        >
          ...
          <p style={{ color: 'var(--muted)', maxWidth: 480, lineHeight: 1.7, marginBottom: 56 }}>
